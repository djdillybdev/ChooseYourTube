...
<nav class="flex items-center justify-between pt-6 pb-2">
	...
	<div class="flex items-center gap-1">
		...
		<!-- First + ellipsis -->
		{#if visiblePages()[0] > 1}
			<button class="btn btn-ghost btn-sm btn-square" onclick={() => navigateTo(1)}>1</button>
			{#if visiblePages()[0] > 2}
		...
		{#if visiblePages()[visiblePages().length - 1] < totalPages}
			{#if visiblePages()[visiblePages().length - 1] < totalPages - 1}
				<span class="px-1 text-base-content/40">…</span>
			{/if}
			<button class="btn btn-ghost btn-sm btn-square" onclick={() => navigateTo(totalPages)}>{totalPages}</button>
		{/if}
	...
	{:else}
		<div></div><!-- spacer so page-size selector stays right-aligned -->
	{/if}
