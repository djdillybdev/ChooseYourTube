<script lang="ts">
	...
	async function handleSave() {
		...
		try {
			...
			actionStatus.announce(`${video.title} saved.`);
			onClose();
		} catch (err) {
...
<DialogShell
	id="save-video-dialog"
	titleId="save-video-title"
	descriptionId="save-video-description"
	boxClass="max-w-2xl"
	busy={isSaving || isCreating}
	{onClose}
>
	<h2 id="save-video-title" class="text-lg font-bold">Save Video to Playlists</h2>
	<p id="save-video-description" class="mt-1 line-clamp-2 text-sm text-base-content/80">
		{video.title}
	</p>
	...
	<form onsubmit={handleCreatePlaylist} class="rounded-box border border-base-300 p-3">
		...
		<div class="grid gap-2 md:grid-cols-[1fr_1fr_auto] md:items-end">
			<label class="form-control text-xs" for="new-playlist-name">
				<span class="mb-1">Playlist name</span>
				<input
			...
			<label class="form-control text-xs" for="new-playlist-description">
	...
	<div class="modal-action">
		<button type="button" class="btn btn-ghost" onclick={onClose} disabled={isSaving || isCreating}
			>Cancel</button
		>
		<button
			type="button"
